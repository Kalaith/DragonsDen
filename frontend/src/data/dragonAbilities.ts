...
export const dragonAbilities: Record<string, DragonAbility> = {
  ...
  poison_immunity: {
    ...
    cooldown: 0,
    manaCost: 0,
    effect: 'Immune to poison, +30% poison damage, regenerates in toxic environments',
    requiredLevel: 9,
    elementType: 'poison'
  },
...
export function getAvailableAbilities(element: ElementType, level: number): DragonAbility[] {
  return getAbilitiesByElement(element).filter(ability => ability.requiredLevel <= level);
}

export function calculateAbilityCooldown(baseTime: number, intelligence: number, level: number): number {
...
export function calculateAbilityDamage(baseDamage: number, stats: { attack: number; magic: number; level: number }): number {
  const attackModifier = stats.attack / 100;
  const magicModifier = stats.magic / 100;
  const levelModifier = 1 + (stats.level * 0.05);
  
  return Math.round(baseDamage * attackModifier * magicModifier * levelModifier);
}