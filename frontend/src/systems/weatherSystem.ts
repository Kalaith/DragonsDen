...
export const weatherEffects: Record<WeatherType, WeatherEffect> = {
  ...
  rain: {
    ...
    specialEffects: ['fire_resistance', 'muddy_terrain', 'enhanced_plant_growth']
  },
  ...
  storm: {
    ...
    specialEffects: ['lightning_strikes', 'strong_winds', 'flight_hazard', 'electrical_interference']
  },
  ...
  fog: {
    ...
    specialEffects: ['reduced_visibility', 'stealth_bonus', 'mystery_encounters']
  },
  ...
  blizzard: {
    ...
    specialEffects: ['freezing_damage', 'extreme_cold', 'ice_terrain', 'visibility_zero']
  },
  ...
  sandstorm: {
    ...
    specialEffects: ['sand_damage', 'equipment_wear', 'buried_treasures', 'navigation_difficulty']
  },
  ...
  eclipse: {
    ...
    specialEffects: ['magical_surge', 'rare_events', 'shadow_creatures', 'mystical_phenomena']
  },
  ...
  aurora: {
    ...
    specialEffects: ['magical_enhancement', 'inspiration_boost', 'navigation_aid', 'beauty_bonus']
  }
};
...
export class WeatherSystemManager {
  ...
  private weatherHistory: Array<{ weather: WeatherType; timestamp: number; duration: number }> = [];
  ...
  private generateNewWeather(biome?: BiomeType): void {
    ...
    possibleWeathers = possibleWeathers.filter(w => !patterns.impossible.includes(w));
    
    // Avoid immediate repetition unless it's a storm system
    const lastWeather = this.weatherHistory[this.weatherHistory.length - 1]?.weather;
    if (lastWeather && possibleWeathers.length > 1) {
      possibleWeathers = possibleWeathers.filter(w => 
  ...
  getElementalModifier(element: ElementType): number {
    const bonus = this.currentWeather.effects.elementalBonuses.find(b => b.element === element);
  ...
  getWeatherForecast(biome?: BiomeType, hoursAhead = 12): Array<{ weather: WeatherType; probability: number }> {
    ...
    const currentWeatherForecast = forecast.find(f => f.weather === this.currentWeather.current);
  ...
  getWeatherDescription(): string {
    const descriptions = {
      clear: 'The sky is crystal clear with perfect visibility. Dragons soar freely through the pristine air.',
      rain: 'Gentle rain falls steadily, creating a soothing rhythm. The air smells fresh and clean.',
      storm: 'Dark clouds rage overhead with fierce winds and crackling lightning. Flight is treacherous.',
      fog: 'A thick, mysterious fog blankets the land, reducing visibility but hiding secrets.',
      blizzard: 'A howling blizzard brings bitter cold and blinding snow. Only the hardiest dare venture out.',
      sandstorm: 'Fierce winds whip sand into a stinging vortex, obscuring all landmarks.',
      eclipse: 'The sun is blocked by shadow, creating an otherworldly twilight filled with mystical energy.',
      aurora: 'Beautiful lights dance across the sky in ribbons of green, blue, and purple.'
    };
  ...
  static getOptimalWeatherFor(activity: string): WeatherType[] {
    ...
    return optimalWeathers[activity as keyof typeof optimalWeathers] || ['clear'];
  }
