  ...
  symbol_sequence: [
    {
      ...
      hints: ['The four primal elements in order of creation', 'Heat before moisture', 'Foundation before freedom'],
      timeLimit: 300
    },
    {
      type: 'symbol_sequence',
      ...
      hints: ['From darkness comes illumination', 'Energy flows through corruption to purity'],
      timeLimit: 480,
  ...
  elemental_alignment: [
    {
      ...
      hints: ['Fire seeks warmth', 'Ice embraces cold', 'Earth anchors all', 'Air rises free'],
      requiredElements: ['fire', 'ice', 'earth', 'air']
    },
    {
      type: 'elemental_alignment',
      difficulty: 6,
      solution: { light: 'zenith', shadow: 'nadir', lightning: 'storm', poison: 'decay' },
      hints: ['Light reaches highest', 'Darkness delves deepest', 'Power crackles in chaos'],
  ...
  pattern_matching: [
    {
      ...
      hints: ['Each number is the sum of the two before', 'Nature loves this sequence'],
      timeLimit: 240
    },
    {
      type: 'pattern_matching',
  ...
  pressure_plates: [
    {
      ...
      difficulty: 4,
      solution: { sequence: ['plate_1', 'plate_3', 'plate_2', 'plate_4'], weight_required: 500 },
      hints: ['Weight of a dragon is needed', 'The path is not straight', 'Begin where the eye looks first'],
  ...
  mirror_reflection: [
    {
      ...
      hints: ['Light bends to will', 'The crystal hungers for illumination', 'Angels matter more than distance'],
  ...
  mechanical_gears: [
    {
      ...
      difficulty: 7,
      solution: { gear_1: 'clockwise', gear_2: 'counter', gear_3: 'clockwise', turns: 7 },
      hints: ['Seven is the number of completion', 'Opposition creates motion', 'The first turns as the sun'],
      timeLimit: 600
    }
  ]
};
  ...
  flame_jet: [
    {
      ...
      duration: 5,
      disarmRequirements: { skill: 'elemental_resistance', difficulty: 4, tools: ['fire_immunity'] }
    }
  ],
...
export class RuinsExplorationSystem {
  static exploreFloor(
    ...
    for (const challenge of floor.challenges) {
      const result = this.processChallenge(challenge, explorationTeam, playerChoices);
    ...
    if (success) {
      ...
      // Guardian encounter
      if (floor.guardian) {
        const guardianResult = this.processGuardian(floor.guardian, explorationTeam);
  ...
  static solvePuzzle(
    ...
    if (!canSolve.success) {
      return {
        ...
        loot: [],
        experience: 10,
        discoveries: canSolve.reason ? [canSolve.reason] : ['Unable to solve puzzle']
      };
    }
    ...
    if (solved) {
      return {
        ...
        discoveries: ['Ancient knowledge unlocked', 'Puzzle mechanism understood']
      };
    } else {
  ...
  static disarmTrap(
    ...
    if (success) {
      return {
        ...
        discoveries: ['Trap disarmed successfully', 'Learned about ancient security']
      };
    } else {
      // Trap triggers
  ...
  static answerRiddle(
    ...
    if (correct) {
      return {
        ...
        discoveries: ['Ancient wisdom gained', 'Riddle master\'s blessing received']
      };
    } else {
      return {
        success: false,
  ...
  static fightGuardian(challenge: RuinChallenge, team: Dragon[]): ExplorationResult {
  ...
  static stealthChallenge(challenge: RuinChallenge, team: Dragon[]): ExplorationResult {
    // Check team stealth capabilities
    const stealthScore = team.reduce((total, dragon) => {
  ...
  private static getBestDragonForTrap(team: Dragon[], trap: TrapConfiguration): Dragon {
  ...
  private static calculateTrapDisarmSkill(dragon: Dragon, trap: TrapConfiguration): number {
    ...
    switch (skill) {
      ...
      case 'mental_resistance':
        skillValue = dragon.stats.intelligence * 1.1 + dragon.stats.defense * 0.3;
        break;
      case 'ancient_knowledge':
        skillValue = dragon.stats.intelligence * 0.8 + (dragon.traits.age === 'ancient' ? 200 : dragon.traits.age === 'elder' ? 100 : 0);
        break;
      default:
        skillValue = 50;
    }
  ...
  private static getPuzzleRewards(difficulty: number): string[] {
    const baseRewards = ['ancient_knowledge_fragment', 'puzzle_solution_scroll'];
