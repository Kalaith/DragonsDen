...
export class RuinsExplorationSystem {
  ...
  static answerRiddle(
    ...
    const playerAnswer =
      typeof playerAnswerValue === 'string' ? playerAnswerValue.toLowerCase() : '';
  ...
  static fightGuardian(challenge: RuinChallenge, team: Dragon[]): ExplorationResult {
    ...
    const teamPower = team.reduce(
      (total, dragon) => total + dragon.stats.attack + dragon.stats.defense + dragon.stats.health,
      0
    );
  ...
  private static generateRiddle(difficulty: number): PuzzleSolution {
    const riddles = puzzleTemplates.riddle.filter(r => Math.abs(r.difficulty - difficulty) <= 1);
    ...
    switch (skill) {
      ...
      case 'ancient_knowledge':
        skillValue =
          ...
          (dragon.traits.age === 'ancient' ? 200 : dragon.traits.age === 'elder' ? 100 : 0);
        break;
      default:
        skillValue = 50;
    }
